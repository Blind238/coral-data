...
const MAX_FILE_SIZE = 90000000 // in bytes, equals 90MB, was previously 100MB but watson often regards the file too large
...
async function zipPending () {
  ...
  for (const dirName of config.idClasses) {
    ...
    const toZip = await selectFromDirUpTo(fileList, config.srcDir + dirName,
      {
...
async function selectFromDirUpTo (files, dir, { maxSize, maxCount }) {
  let toZip = []
  let totalBytes = 0

  for (const fileName of files) {
    let stats = await fs.stat(dir + '/' + fileName)
    if (stats.size + totalBytes < maxSize) {
