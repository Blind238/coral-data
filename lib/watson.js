...
module.exports = {
  listClassifiers,
  updateClassifiers,
  // does not seem to integrate well into watson studio
  // it does work, classes only visible via API
  createClassifiers
}
...
async function createClassifiers () {
  let zipLists = await availableZipsToTrain()
  ...
  // this sends our requests all at once
  // let paramPromises = Object.keys(params).map(async (name) => {
  ...
  for (const name of Object.keys(params)) {
    params[name].name = name
    try {
      responses[name] = await vr.createClassifier(params[name])
    } catch (error) {
      responses[name] = error
    }
    console.log(JSON.stringify(responses[name], null, 2))
  }
...
function assignZipsToClassifiers (zipLists, classifiers) {
  ...
  for (const classifier of classifiers) {
    ...
    for (const zipListName of Object.keys(zipLists)) {
      // Below rules don't need to apply anymore, machine learning is smart enough
