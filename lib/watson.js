...
function canTrain (classifierResponse) {
  ...
  return classifierResponse &&
         classifierResponse.classifiers &&
         classifierResponse.classifiers.length !== 0 &&
         classifierResponse.classifiers[0].status === 'ready'
...
async function createTrainingRequest (zips, classifierResponse) {
  ...
  for (const dir of config.idClasses) {
    if (zips[dir] !== undefined && dir === classifierResponse.classifiers[0].name) {
      params[dir + '_positive_examples'] = fs.createReadStream(config.zipDir + dir + '/' + zips[dir][0])
    }
  }
...
async function createClassifiers (zips) {
  let params = {}
  // ensure we have a zip for each class we want to train
  if (Object.keys(zips).length !== config.idClasses.length) {
    throw new Error('amount of zips need to equal classes to create')
  }
